...
# project is indexed. The result is cached for the session in
# $CLAUDE_PLUGIN_DATA/cix-aware-$SESSION_ID so the PreToolUse hook can
# short-circuit without re-querying the server.
#
...
#
# Decision contract (read by grep-nudge.sh):
...
#   File absent                   → SessionStart didn't run, nudge stays silent
#
# Why no fallback in grep-nudge: if SessionStart concluded "not indexed"
# (server unreachable, project not registered, etc.), the user should
# NOT see Grep nudges suggesting `cix search` for the rest of the
# session. Sending nudges based on `.cixignore` presence anyway would
