...
**Step 1 — register the cix MCP server (required first):**
```
cix mcp install claude-desktop
```
This registers the `cix mcp` server in Claude Desktop (one command, no
clicking). After restarting Claude Desktop you have the `cix_*` tools, and the
server's built-in `instructions` already give the agent the essentials.
...
> You can stop at Step 1: the MCP server alone works, with condensed guidance.
> Step 2 is the upgrade for heavy cross-repo research, where the `cix-workspace`
> skill's detail pays off.

## Requirements

- The cix MCP server registered (Step 1), which needs the `cix` binary on PATH
